...
import com.bbpay.common.bean.Device;
    ...
    public static void handleInfo(final Context context, AbstractResponse response) {
        if (response != null) {
            ...
            if (CollectionUtils.isNotEmpty(response.params)) {
                for (com.bbpay.common.bean.KeyValue kv : response.params) {
                    onLineParams.put(kv.key, kv.value);
                }
            }
        }
    }
    public static <T extends com.bbpay.common.bean.request.AbstractForm> T initForm(Context context, Class<T> c) throws Exception {
